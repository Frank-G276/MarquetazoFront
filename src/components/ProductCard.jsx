...
export function ProductCard({ product }) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <Link to={`/producto/${product._id}`} className="block aspect-[4/3] overflow-hidden bg-white no-underline">
        {image ? (
          <img src={image} alt="" className="h-full w-full object-contain p-2 transition duration-300 group-hover:scale-105" />
        ) : (
